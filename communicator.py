...
class Communicator:
    ...
    def __validate_params(self):
        ...
        valid_methods = ['plain', 'choco', 'dcd_psgd', 'ecd_psgd']
        ...
        valid_topologies = ['disconnected', 'path', 'star', 'ring', 'complete', 'barbell']
        ...
        # Check if given transition matrix for topology is valid
            ...
            if self.topology.n_dim != 2 or n_rows != n_cols:
    ...
    def __validate_transition_matrix(self):

        # If the transition matrix is to be generated given a String
        if isinstance(self.topology, str):
            # TODO implement networkx graph generation + add self-loops

            # Add self-loops
            adjacency_matrix = adjacency_matrix + np.eye(adjacency_matrix.shape[0])
            return None

        # Else the transition matrix is directly given
        else:
            transition_matrix = self.topology

        # Get the number of
        n_rows, _ = adjacency_matrix.shape

        # Make sure that the rows of the matrix are normalized
        if not np.allclose(adjacency_matrix.sum(axis=1), np.ones(n_rows)):
    ...
    def communicate(self, weight_matrix):
        ...
        elif self.method == 'dcd_psgd':
            return __communicate_dcd_psgd(weight_matrix)
        elif self.method == 'ecd_psgd'
            return __communicate_ecd_psgd(weight_matrix)

    def __communicate_plain(weight_matrix):
        return None

    def __communicate_choco(weight_matrix):
        return None

    def __communicate_dcd_psgd(weight_matrix):
        return None

    def __communicate_ecd_psgd(weight_matrix):
        return None
